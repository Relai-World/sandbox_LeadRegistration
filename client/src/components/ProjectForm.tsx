...
export const normalizePropertyData = (property: any): FormData => {
  ...
  const registrationModes = mapRegistrationModes(property.Accepted_Modes_of_Lead_Registration || property.accepted_modes_of_lead_registration);
  ...
  const result = {
    ...
    secondary: {
      ...
      leadRegistrationRequired: property.Is_Lead_Registration_Required_Before_Site_Visit || property.Is_lead_Registration_required_before_Site_visit || property.is_lead_registration_required_before_site_visit || '',
      leadAcknowledgementTime: property.Turnaround_Time_for_Lead_Acknowledgement || property.turnaround_time_for_lead_acknowledgement || '',
      validityPeriod: property.Is_There_Validity_Period_for_Registered_Lead || property.Is_there_validity_period_for_registered_lead || property.is_there_validity_period_for_registered_lead || '',
      validityPeriodValue: property.Validity_Period_Value || property.validity_period_value || '',
      ...registrationModes,
      leadRegistrationNotes: property.Notes_Comments_on_Lead_Registration_Workflow || property.Notes_Comments_on_lead_registration_workflow || property.notes_comments_on_lead_registration_workflow || '',
...
export const ProjectForm: React.FC<ProjectFormProps> = ({
  ...
  const convertToApiFormat = (formData: FormData) => {
    ...
    return {
      ...
      configurations: (units.configurations || []).map((config: any) => ({
        ...config,
  ...
  const handleNext = () => {
    const tabOrder = ['basics', 'construction', 'units', 'financial', 'secondary'];
